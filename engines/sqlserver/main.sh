...
validate_sqlserver_env() {
    ...
    [[ "$has_upper" == "true" ]] && ((complexity_count++))
    [[ "$has_lower" == "true" ]] && ((complexity_count++))
    [[ "$has_digit" == "true" ]] && ((complexity_count++))
    [[ "$has_special" == "true" ]] && ((complexity_count++))
...
wait_for_sqlserver() {
    ...
    while [[ $attempt -le $max_attempts ]]; do
        ...
        if exec_container "$container_name" /opt/mssql-tools/bin/sqlcmd \
...
create_initial_database() {
    ...
    local db_exists
    db_exists=$(exec_container "$container_name" /opt/mssql-tools/bin/sqlcmd \
        -S localhost -U sa -P "$sa_password" \
